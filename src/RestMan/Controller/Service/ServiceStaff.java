...
import RestMan.Model.ModelCTNK;
import RestMan.Model.ModelCTXK;
import RestMan.Model.ModelHoaDon;
import RestMan.Model.ModelKhachHang;
import RestMan.Model.ModelKho;
import RestMan.Model.ModelNguyenLieu;
import RestMan.Model.ModelNhanVien;
import RestMan.Model.ModelPNK;
import RestMan.Model.ModelPXK;
...
public class ServiceStaff {
    ...
    public ModelNhanVien getStaff(int userID) throws SQLException {
        ...
        String sql = "SELECT ID_NV, TenNV, DATE_FORMAT(NgayVL, 'dd-mm-yyyy') AS Ngay, SDT, Chucvu, ID_NQL FROM NhanVien WHERE ID_ND=?";
    ...
    public ModelPNK getPNKbyID(int id) throws SQLException {
        ModelPNK data = null;
        String sql = "SELECT ID_NK,ID_NV,DATE_FORMAT(NgayNK,'dd-mm-yyyy') AS Ngay,Tongtien FROM PhieuNK WHERE ID_NK=?";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, id);
        ResultSet r = p.executeQuery();
        while (r.next()) {
    ...
    public ArrayList<ModelPXK> MenuPXK() throws SQLException {
        ...
        String sql = "SELECT ID_XK,ID_NV,DATE_FORMAT(NgayXK,'dd-mm-yyyy') AS Ngay FROM PhieuXK ORDER BY ID_XK";
        PreparedStatement p = con.prepareStatement(sql);
        ResultSet r = p.executeQuery();
        while (r.next()) {
            int idXK = r.getInt(1);
    ...
    public ModelPXK getPXKbyID(int id) throws SQLException {
        ModelPXK data = null;
        String sql = "SELECT ID_XK,ID_NV,DATE_FORMAT(NgayXK,'dd-mm-yyyy') AS Ngay FROM PhieuXK WHERE ID_XK=?";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, id);
        ResultSet r = p.executeQuery();
        while (r.next()) {
    ...
    public ArrayList<ModelKhachHang> MenuKH() throws SQLException {
        ...
        String sql = "SELECT ID_KH, TenKH, DATE_FORMAT(Ngaythamgia,'dd-mm-yyyy') AS Ngay, Doanhso, Diemtichluy FROM KhachHang";
        PreparedStatement p = con.prepareStatement(sql);
        ResultSet r = p.executeQuery();
        while (r.next()) {
    ...
    public ModelHoaDon FindHoaDonbyID_Ban(ModelBan table) throws SQLException {
        ...
        String sql = "SELECT ID_HoaDon,ID_KH,ID_Ban,DATE_FORMAT(NgayHD,'dd-mm-yyyy') AS Ngay,TienMonAn,Code_Voucher,TienGiam,Tongtien,Trangthai FROM HoaDon "
