...
import RestMan.Model.ModelVoucher;
...
public class ServiceCustomer {
    ...
    public ModelKhachHang getCustomer(int userID) throws SQLException {
        ...
        String sql = "SELECT ID_KH, TenKH, DATE_FORMAT(Ngaythamgia, 'dd-mm-yyyy') AS NgayTG, Doanhso,Diemtichluy FROM KhachHang WHERE ID_ND=?";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, userID);
        ResultSet r = p.executeQuery();
        while (r.next()) {
    ...
    public void InsertHoaDon(ModelBan table, ModelKhachHang customer) throws SQLException {
        // Tìm ID_HD tiếp theo
        int idHD = 1; // Bắt đầu từ 1 nếu bảng HoaDon rỗng
        String findMaxIDQuery = "SELECT IFNULL(MAX(ID_HoaDon), 0) + 1 FROM HoaDon";
        PreparedStatement p_ID = con.prepareStatement(findMaxIDQuery);
        ...
        String sql = "INSERT INTO HoaDon(ID_HoaDon, ID_KH, ID_Ban, NgayHD, TienMonAn, TienGiam, Trangthai)"
                + " VALUES (?, ?, ?, CURRENT_DATE(), 0, 0, 'Chua thanh toan')";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, idHD);
    ...
    public ArrayList<ModelHoaDon> getListHDOrder(int ID_KH, String order) throws SQLException {
        ...
        switch (order) {
            case "Tất cả":
                sql = "SELECT ID_HoaDon,ID_KH,ID_Ban,DATE_FORMAT(NgayHD,'dd-mm-yyyy') AS Ngay,TienMonAn,Code_Voucher,TienGiam,Tongtien,Trangthai FROM HoaDon "
                        + "WHERE ID_KH=? ORDER BY ID_HoaDon";
                break;
